import { effectiveSessionStatus, milestoneWeek, sessionDate } from "./data.mjs";
import { ruleForDate, communityAmount } from "./rules.mjs";
import { activeServerteam } from "./serverteam.mjs";

export function weeklyPreview(state, month, now = Date.now()) {
  ...
  for (const session of state.snapshot?.sessions ?? []) {
    if (effectiveSessionStatus(state, session) !== "played") continue;
    const week = milestoneWeek(session.startTime, rule.timeZone);
    if (week.month !== month) continue;
    const start = new Date(`${sessionDate(session.startTime, rule.timeZone)}T00:00:00Z`);
